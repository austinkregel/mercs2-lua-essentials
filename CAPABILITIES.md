...
| `Ess.Spawn` | Put **many** things in the world at once — the bulk form of `Ess.Object.spawn` | `.many(templates, count, opts)` — `templates` is one name **or a roster array** (`{"AL Soldier", "AL Heavy"}`); omit `count` with a roster to spawn one of each, give one and the roster repeats keeping its ratio exact. `opts`: **where** `at={x,y,z}`/`around=guid`/`ahead=dist` (default 20 ahead), **shape** `pattern="scatter"`(default, `minDist`/`maxDist`)/`"grid"`/`"line"`/`"circle"` (`spacing`), **each** `yaw`/`faceCentre`/`faceAway`/`height`/`snapToGround`, **draw** `pick="cycle"`(default)/`"random"`/`seed`, **admin** `tracker`/`onEach`/`max` (default cap 64); `.mixed({{template,count},…}, opts)` (exact counts, not a ratio); `.at(templates, points, opts)` (one per explicit `{x,y,z}`). ⚠ **Every template is validated BEFORE anything spawns** — one bad entry in a roster spawns *nothing*, because a blank template hard-CTDs past `pcall` and a half-applied bulk spawn can't be undone. `scatter` sectors its angles so two units can never land on the same spot. **Easy:** `Ess.Easy.Spawn.units/.vehicles/.props(n, template)` and `.roster(templates, qty, minDist, maxDist)`; in a menu action, `ctx:spawnArray(templates, qty, min, max)` |
...
| `Ess.Ecs` | The engine's ECS component-class **registry** as a typed vocabulary | The ~232 reflection component classes an entity is assembled from (`RuntimeHealth`, `StateMachine`, `Explosive`, `AiPatrol`, …), in 9 families, each with its component **hash** (`pandemic_hash_m2(name)` — what the engine's resolver keys on). `.classes()`, `.get(name)`, `.hash(name)`, `.family(name)`, `.find(query)` (name/family substring), `.families()`. This is the *naming* half of "what is a live entity made of" — a generic **raw** per-entity component read still needs a native memory-read verb (the object→component resolver + entity slot table are reversed; these hashes are its keys), so `Ess.Inspect` reads the components exposed via getters for now |
| `Ess.Inspect` | A structured, NAMED read of an entity — the "remote inspector" | `Ess.Inspect.read(guid)` (or `Ess.Inspect(guid)`) → a typed record grouped like the engine's components (identity/transform/health/physics/vehicle/faction), each field pulled through its confirmed getter and guarded (absent, not an error, when the engine won't answer); `.print(guid)` (log it grouped, for the console), `.line(guid)` (one-line summary). Recovers what nothing else can: a readable **name**/**model** — `Object.GetName`/`GetModelName` return an opaque handle, but it stringifies via `Sys.GuidToString` and `Ess.Names` reverses the hash (live: a spawned Veyron's model → `civ_veh_car_veyron`); degrades to the bare `0x…` without the names table |
| `Ess.Names` | Reverse a `0xHASH` back to the name it was hashed from | `.of(hash)` (name, or **nil on a miss — never a guessed name**), `.label(hash)` (`"name (0xHASH)"`, or the bare hash when unknown — always safe to log), `.installed()`, `.count()`, `.load(table)`; **`Ess.Named(guid)`** = `Ess.Name` with the meaning put back (`"refinery_doc_warehouse01 (0x4000563D)"`). Needs the **optional** ~23k-entry table — as a Shipment, `import("ess_names")`; on an OnLoad install, `scripts/OnLoad/2_EssNames.lua`, opt-in via a loader line (see `Ess-README.txt` in the OnLoad zip); without it every call degrades to nil / the bare hash. Keys are the string `"0x…"` form deliberately, because Lua 5.1's 32-bit floats can't hold a high hash exactly |
| `Ess.Machine` | The object **destruction / state machine** as a live control surface | `.set(guid, node, state)` (force a node to a state — name or `0xHASH`; a state outside the global vocabulary is refused, not silently dead), `.link(guid, hardpoint)` (resolve a linked sub-part — `ObjectState.GetLinkGuid`), `.onChange(fn)` → `stop()` (`fn(guid, sState, sNode)` on every transition, hashes resolved to names via the vocabulary + `Ess.Names`; **chains** any existing `OnStateChange`), `.name(hash)` (state hash → vocabulary name, else bare hash), `.print(guid)`, `.STATES`/`.vocab()` — the cracked global vocabulary (`PristineState`/`DamagedState`/`DestroyedState`/`CollapseState`/…). Distinct from `Ess.State` (persistence) and `Ess.Human.setState` (posture). **Smoke-tested live** over the bridge — `.set()` drove a real building's 8 nodes to `DestroyedState` and the engine reported each back through `.onChange`. Note `.set()` is a **logical** state change (object stays alive; visible destruction is the damage path / `Ess.Object.kill`) (`resident/oilrig.lua`) |
